...
import AddNewUserDrawer from "@/views/apps/quanliphongthinghiem/khonguyenlieu/AddNewNguyenLieu.vue";
...
const headersDevices = [
  { title: "Tên nguyên vật liệu", key: "name" },
  { title: "Mã số ", key: "code" },
  { title: "Số lượng", key: "quantity" },
  { title: "Ngày nhập kho", key: "inputDay" },
  { title: "Hạn sử dụng", key: "expiry" },
  { title: "Người nhập kho", key: "warehouseImporter" },
  { title: "Thông tin liên hệ nhà cung cấp", key: "SupplierContactInformation" },
  { title: "Actions", key: "actions", sortable: false },
];

// 👉 Fetching device
const { data: deviceData, execute: fetchDevice } = await useApi<any>(
  createUrl("/materials/find-all", {
    query: {
      q: searchQuery,
...
let inputDay = ref('')
let expiry = ref()
let warehouseImporter = ref('')
let SupplierContactInformation = ref('')

// 👉 Add new user
const addNewUser = async (userData: UserProperties) => {
  await $api("/apps/materials", {
    method: "POST",
    body: userData,
  });
...
const deleteUser = async (id: number) => {
  await axios.delete(`${baseUrl}/v1/materials/${id}`);
  // refetch User
...
const getDeviceDetail = async (id: number) => {
  const response = await axios.get(`${baseUrl}/v1/materials/${id}`);
  deviceDetail = { ...response.data.data };
  name = ref(deviceDetail.name)
  code = ref(deviceDetail.code)
  quantity = ref(deviceDetail.quantity)
  inputDay = ref(deviceDetail.inputDay)
  expiry = ref(deviceDetail.expiry)
  warehouseImporter = ref(deviceDetail.warehouseImporter)
  SupplierContactInformation = ref(deviceDetail.SupplierContactInformation)
};
  ...
  <section>
    ...
    <VCard class="mb-6">
      <VCardItem class="pb-4">
        <VCardTitle>Danh sách nguyên liệu</VCardTitle>
      </VCardItem>
      ...
      <VDataTableServer v-model:items-per-page="itemsPerPage" v-model:page="page" :items="devices"
        ...
        <template #item.warehouseImporter="{ item }">
          <div class="text-body-1 text-high-emphasis text-capitalize">
            {{ item.warehouseImporter }}
          </div>
        </template>

        <template #item.SupplierContactInformation="{ item }">
          <div class="text-body-1 text-high-emphasis text-capitalize">
            {{ item.SupplierContactInformation }}
          </div>
        </template>
          ...
          <VCardText>
            ...
            <VForm ref="refForm" @submit.prevent="onSubmit">
              <VRow>
                ...
                <!-- 👉 Email -->
                <VCol cols="12">
                  <AppTextField v-model="quantity" :rules="[requiredValidator]" label="Số lượng"
